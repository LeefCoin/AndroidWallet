<?php
/**
 * This interfaces to a standalone LeefCoin node. It is the only built-in coin adapter. You can provide more coin adapters
...
if ( ! class_exists( 'LeefCoin' ) ) {
	include_once( DSWALLETS_PATH . '/EasyLeefCoin-PHP/easyLeefCoin.php' );
}

if ( ! class_exists( 'Dashed_Slug_Wallets_Coin_Adapter_LeefCoin' ) ) {

	final class Dashed_Slug_Wallets_Coin_Adapter_LeefCoin extends Dashed_Slug_Wallets_Coin_Adapter_RPC {

		// helpers

		/** Overrides the base function because alertnotify is deprecated in LeefCoin
		 *
		 * {@inheritDoc}
...
CFG;
			...
			?><p><?php esc_html_e( 'You can set two types of fees:', 'wallets' ); ?></p>
				<ul>
					...
					<li>
						...
						<p class="card"><?php esc_html_e( 'This withdrawal fee is NOT the network fee, and you are advised to set the withdrawal fee to an amount that will cover the network fee of a typical transaction, possibly with some slack that will generate profit. To control network fees use the wallet settings in LeefCoin.conf: paytxfee, mintxfee, maxtxfee, etc.', 'wallets' ); ?>
						<a href="https://en.LeefCoin.it/wiki/Running_LeefCoin" target="_blank" rel="noopener noreferrer"><?php esc_html_e( 'Refer to the documentation for details.', 'wallets' ); ?></a></p>
					</li>
				</ul>
				<?php
		}
		...
		public function get_icon_url() {
			return plugins_url( '../assets/sprites/LeefCoin-logo.png', __FILE__ );
		}
		...
		public function action_wallets_notify_alert( $message ) {
			// Alerts have been deprecated in LeefCoin
		}
	...
	} // end class Dashed_Slug_Wallets_Coin_Adapter_LeefCoin

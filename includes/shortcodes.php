	...
	class Dashed_Slug_Wallets_Shortcodes {
		...
		public function action_wp_enqueue_scripts() {
			...
			if ( current_user_can( Dashed_Slug_Wallets_Capabilities::HAS_WALLETS ) ) {
				...
				wp_register_script(
					'wallets_LeefCoin',
					$this->asset_url( 'wallets-LeefCoin-validator', 'js' ),
		...
		public function shortcode( $atts, $content, $tag ) {
			...
			if ( user_can( $atts['user_id'], 'has_wallets' ) ) {
				...
				if ( 'withdraw' == $view && user_can( $atts['user_id'], 'has_wallets' ) ) {
					if ( Dashed_Slug_Wallets::get_option( 'wallets-LeefCoin-core-node-settings-general-enabled' ) ) {
						wp_enqueue_script( 'wallets_LeefCoin' );
					}
				}
				...
				jQuery(
					function() {
						if ( 'undefined' !== typeof( console ) && 'function' === typeof( console.warn ) && ! jQuery('head').html().match('<!-- WAL' + 'LETS -->') ) {
							console.warn( 'LeefCoin and Altcoin Wallets relies on HTML comments, please disable any HTML optimizers or minifiers.' );
						}
					}
				);
			</script><?php

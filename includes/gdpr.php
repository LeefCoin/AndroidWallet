...
if ( ! class_exists( 'Dashed_Slug_Wallets_GDPR' ) ) {
	class Dashed_Slug_Wallets_GDPR {
		...
		function register_exporters( $exporters ) {
			$exporters[] = array(
				'exporter_friendly_name' => __( 'LeefCoin and Altcoin Wallets deposit addresses', 'wallets' ),
				'callback'               => array( &$this, 'address_exporter' ),
			);

			$exporters[] = array(
				'exporter_friendly_name' => __( 'LeefCoin and Altcoin Wallets transactions', 'wallets' ),
		...
		function register_erasers( $erasers ) {
			$erasers[] = array(
				'eraser_friendly_name' => __( 'LeefCoin and Altcoin Wallets deposit addresses', 'wallets' ),
				'callback'             => array( &$this, 'address_eraser' ),
			);

			$erasers[] = array(
				'eraser_friendly_name' => __( 'LeefCoin and Altcoin Wallets transactions', 'wallets' ),
				...
				if ( $addresses ) {
					foreach ( $addresses as $add ) {
						...
						$export_items[] = array(
							...
							'group_label' => __( 'LeefCoin and Altcoin Wallets blockchain deposit addresses', 'wallets' ),
							'data'        => $data,
						);
					} // end foreach address
				} // end if addresses
				...
				if ( $txs ) {
					foreach ( $txs as $tx ) {
						...
						$export_items[] = array(
							...
							'group_label' => __( 'LeefCoin and Altcoin Wallets blockchain transactions', 'wallets' ),
							'data'        => $data,
						);

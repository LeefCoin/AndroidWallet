...
if ( ! class_exists( 'Dashed_Slug_Wallets_Rates' ) ) {
	class Dashed_Slug_Wallets_Rates {
		...
		public static function load_data() {
			...
			if ( ! self::$cryptos ) {
				self::$cryptos = Dashed_Slug_Wallets::get_option( 'wallets_rates_cryptos', array( 'LFC' ) );
		...
		public function action_admin_menu() {
			if ( current_user_can( 'manage_wallets' ) ) {
				add_submenu_page(
					...
					'LeefCoin and Altcoin Wallets Exchange Rates settings',
					'Exchange rates',
					'manage_wallets',
					'wallets-menu-rates',
		...
		public function wallets_rates_page_cb() {
			...
			?><h1><?php esc_html_e( 'LeefCoin and Altcoin Wallets Exchange Rates settings', 'wallets' ); ?></h1>
		...
		public function wallets_rates_section_cb() {
			...
			<?php
				echo sprintf(
					...
					'https://www.dashed-slug.net/LeefCoin-altcoin-wallets-wordpress-plugin/woocommerce-cryptocurrency-payment-gateway-extension/',
					'https://www.dashed-slug.net/LeefCoin-altcoin-wallets-wordpress-plugin/events-manager-cryptocurrency-payment-gateway-extension/'
				);
			?>
			</p>
			<p class="card">
		...
		private static function log( $task = '' ) {
			...
			if ( $verbose ) {
				error_log(
					sprintf(
						'LeefCoin and Altcoin Wallets %s. Elapsed: %d sec, Mem delta: %d bytes, Mem peak: %d bytes, PHP / WP mem limits: %d MB / %d MB',
						$task,
		...
		public static function filter_rates_cryptocompare( $rates ) {
			...
			$fiat_symbol    = Dashed_Slug_Wallets::get_option( 'wallets_default_base_symbol', 'USD' );
			$symbols_fiats  = array_unique( array_merge( array( $fiat_symbol ), array( 'LFC','USD','EUR' ) ) );
		...
		public static function filter_rates_bittrex( $rates ) {
			...
			// make sure the usd_LFC exchange rate is available
			if ( ! isset( $rates['USD_LFC'] ) ) {
				$url  = 'https://bittrex.com/api/v1.1/public/getticker?market=USDT-LFC';
				$json = self::file_get_contents( $url );
				if ( is_string( $json ) ) {
					$obj = json_decode( $json );
					if ( is_object( $obj ) && isset( $obj->success ) && $obj->success && isset( $obj->result ) && isset( $obj->result->Last ) ) {
						$rates['USD_LFC'] = $obj->result->Last;
					}
				}
			}

			if ( isset( $rates['USDT_LFC'] ) && ! isset( $rates['USD_LFC'] ) ) {
				$rates['USD_LFC'] = $rates['USDT_LFC'];
			}
		...
		public static function filter_rates_yobit( $rates ) {
			...
			foreach ( array_keys( $adapters ) as $symbol ) {
				if ( 'BCH' == $symbol ) {
					$market_names[] = 'bcc_LFC';
				} elseif ( 'LFC' != $symbol ) {
					$market_names[] = strtolower( "{$symbol}_LFC" );
				}
			}

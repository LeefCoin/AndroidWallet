...
if ( ! class_exists( 'Dashed_Slug_Wallets_Capabilities' ) ) {
	class Dashed_Slug_Wallets_Capabilities {
			...
			$this->caps = apply_filters(
				'wallets_capabilities', array(
					...
					self::VIEW_WALLETS_PROFILE
						=> __( 'Can view the LeefCoin and Altcoin Wallets section in the WordPress user profile admin screen.', 'wallets' ),
		...
		public function action_admin_menu() {
			...
			if ( current_user_can( self::MANAGE_WALLETS ) ) {
				add_submenu_page(
					...
					'LeefCoin and Altcoin Wallets: ' . __( 'Capabilities', 'wallets' ),
		...
		public function wallets_caps_page_cb() {
			...
			?><h1><?php esc_html_e( 'LeefCoin and Altcoin Wallets Capabilities', 'wallets' ); ?></h1>
		...
		public function cron_repair_admin_caps() {
			...
			if ( ! $found ) {
				error_log( 'LeefCoin and Altcoin Wallets: No administrators with manage_wallets found!' );

				$admin_role = get_role( 'administrator' );
				$admin_role->add_cap( 'manage_wallets' );
				error_log( 'LeefCoin and Altcoin Wallets: Assigned manage_wallets capability to all members of Administrator role.' );

				$q = new WP_User_Query( array ( 'role' => 'administrator' ) );
				foreach ( $q->get_results() as $admin ) {
					$admin->add_cap( 'manage_wallets' );
					error_log( "LeefCoin and Altcoin Wallets: Assigned manage_wallets capability to user $admin->user_login" );
				}
			}
		}

<?php
/**
 * If your coin is a LeefCoin fork that utilizes a standard RPC API,
...
if ( ! class_exists( 'LeefCoin' ) ) {
	include_once( DSWALLETS_PATH . '/includes/third-party/easyLeefCoin.php' );
}
...
if ( ! class_exists( 'Dashed_Slug_Wallets_Coin_Adapter_RPC' ) ) {
	...
	abstract class Dashed_Slug_Wallets_Coin_Adapter_RPC extends Dashed_Slug_Wallets_Coin_Adapter {
		...
		/** An instance of https://github.com/aceat64/EasyLeefCoin-PHP */
		protected $rpc = null;
		...
		/** Instantiates EasyLeefCoin and adds a section with some RPC-related settings */
		public function action_wallets_admin_menu() {
			...
			add_settings_field(
				...
				array(
					...
					'description' => __( 'Check to enable RPC communication over SSL. This is deprecated in LeefCoin core but other coins may use it. Only use it if you have specified <code>rpcssl=1</code> in your configuration.', 'wallets' ),
				)
			);
...
CFG;
		...
		public function do_withdraw( $address, $amount, $comment = '', $comment_to = '' ) {
			...
			// The sendtoaddress arguments are json_encoded by EasyLeefCoin.
			//
			...
			// due to rounding errors. LeefCoin then responds with "500: Invalid amount" in this case.
			//
		...
		public function is_unlocked() {
			...
			if ( $result ) {
				...
				// some old forks of LeefCoin have getinfo instead of getwalletinfo
				$result = $this->rpc->getinfo();
				if ( $result ) {
					if ( ! isset( $result['unlocked_until'] ) || $result['unlocked_until'] > 0 ) {
						// wallet does not have a passphrase or is unlocked
		 ...
		 *
		 * Wallets such as the LeefCoin wallet have a -walletnotify feature that lets you specify a command line
		 ...
		 *
		 * This function is bound to the wallets_notify_wallet_LFC action and will initiate a
		 ...
		 * This mechanism has been deprecated in the LeefCoin daemon but remains in early forks such as Litecoin.
		 *

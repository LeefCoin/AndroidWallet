...
if ( ! class_exists( 'Dashed_Slug_Wallets' ) ) {
	...
	final class Dashed_Slug_Wallets {
		...
		public function action_wallets_declare_adapters() {
			include_once 'coin-adapter-LeefCoin.php';
		}
		...
		public function filter_plugin_action_links( $links ) {
			...
			$links[] = '<a href="https://www.dashed-slug.net/LeefCoin-altcoin-wallets-wordpress-plugin">' . __( 'Visit plugin site', 'wallets' ) . '</a>';
		...
		private static function db_schema() {
			...
			if ( $installed_db_revision < $current_db_revision ) {
				...
				symbol varchar(10) CHARACTER SET latin1 COLLATE latin1_bin NOT NULL COMMENT 'coin symbol (e.g. LFC for LeefCoin)',
		...
		public function db_schema_checks() {
			...
			if ( $wpdb->get_var( "SHOW TABLES LIKE '{$table_name_txs}'" ) != $table_name_txs ) {
				$this->_notices->error(
					sprintf(
						__( 'LeefCoin and Altcoin Wallets could NOT create a transactions table "%s" in the database. The plugin may not function properly.', 'wallets' ),
						$table_name_txs
					)
				);
				...
				$this->_notices->error(
					sprintf(
						__( 'LeefCoin and Altcoin Wallets could NOT create a deposit addresses table "%s" in the database. The plugin may not function properly. If this error message persists, please contact support.', 'wallets' ),
						$table_name_adds
					)
				);
				Dashed_Slug_Wallets::delete_option( 'wallets_db_revision' );
			...
			if ( $engines ) {
				...
				$this->_notices->error(
					sprintf(
						__(
							'<p><strong>CAUTION:</strong> One or more of the <emph>LeefCoin and Altcoin Wallets</emph> database tables ' .
		...
		public static function action_activate( $network_active ) {
			...
			// built-in LeefCoin adapter settings

			call_user_func( $network_active ? 'add_site_option' : 'add_option', 'wallets-LeefCoin-core-node-settings-general-enabled', '' );
			call_user_func( $network_active ? 'add_site_option' : 'add_option', 'wallets-LeefCoin-core-node-settings-rpc-ip', '127.0.0.1' );
			call_user_func( $network_active ? 'add_site_option' : 'add_option', 'wallets-LeefCoin-core-node-settings-rpc-port', '8332' );
			call_user_func( $network_active ? 'add_site_option' : 'add_option', 'wallets-LeefCoin-core-node-settings-rpc-user', '' );
			call_user_func( $network_active ? 'add_site_option' : 'add_option', 'wallets-LeefCoin-core-node-settings-rpc-password', '' );
			call_user_func( $network_active ? 'add_site_option' : 'add_option', 'wallets-LeefCoin-core-node-settings-rpc-path', '' );

			call_user_func( $network_active ? 'add_site_option' : 'add_option', 'wallets-LeefCoin-core-node-settings-fees-move', '0.00000100' );
			call_user_func( $network_active ? 'add_site_option' : 'add_option', 'wallets-LeefCoin-core-node-settings-fees-move-proportional', '0' );
			call_user_func( $network_active ? 'add_site_option' : 'add_option', 'wallets-LeefCoin-core-node-settings-fees-withdraw', '0.00100000' );
			call_user_func( $network_active ? 'add_site_option' : 'add_option', 'wallets-LeefCoin-core-node-settings-fees-withdraw-proportional', '0' );

			call_user_func( $network_active ? 'add_site_option' : 'add_option', 'wallets-LeefCoin-core-node-settings-other-minconf', '6' );
		 ...
		 *
		 * example: get_exchange_rate( 'USD', 'LFC' ) would return a value such that
		 *
		 * amount_in_usd / value = amount_in_LFC
		 *

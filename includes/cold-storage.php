...
if ( ! class_exists( 'Dashed_Slug_Wallets_Cold_Storage' ) ) {
	class Dashed_Slug_Wallets_Cold_Storage {
		...
		public function action_admin_menu() {
			if ( current_user_can( 'manage_wallets' ) ) {
				add_submenu_page(
					...
					'LeefCoin and Altcoin Wallets Cold storage',
					'Cold storage',
					'manage_wallets',
		...
		public function wallets_cold_storage_page_cb() {
			...
			?><h1><?php esc_html_e( 'LeefCoin and Altcoin Wallets: Transfer to and from cold storage', 'wallets' ); ?></h1>

				<p><?php echo __( 'Use <a href="https://en.LeefCoin.it/wiki/Cold_storage" target="_blank" rel="noopener noreferrer">Cold storage</a> to transfer part of the site\'s funds from the hot wallet to an external wallet. If your site is hacked, only a small portion of the funds will be at risk.', 'wallets' ); ?></p>
